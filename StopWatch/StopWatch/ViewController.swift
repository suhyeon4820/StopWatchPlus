...
class ViewController: UIViewController {
    ...
    @objc private func didTapStartButton(_sender: Any) {
        if timerCounting {
            ...
            // Pause 화면
            timerCounting = true
            stopWatch.timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(timeCounter), userInfo: nil, repeats: true)
    ...
    @objc private func timeCounter() -> String {
        ...
        let minute = Int(((stopWatch.count % 3600)/60)/60) // count min으로 전환
        let second = Int((stopWatch.count % 3600)/60)      // count second로 전환
        let miliSecond = Int(stopWatch.count % 60)         // count milisecond로 전환
